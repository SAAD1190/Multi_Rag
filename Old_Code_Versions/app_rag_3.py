...
# Initialize FAISS (for Arabic with SentenceTransformer)
...
if language == "English":
    ...
    st.title("منصة الدردشة متعددة اللغات")
    sidebar_title = "الإعدادات"
    upload_header = "تحميل المستندات (PDF فقط)"
    ...
    if query.strip():
        try:
            if language == "العربية":
                ...
                # English: Use selected Llama model with Chroma
            ...
            st.error(f"An error occurred: {e}" if language == "English" else f"حدث خطأ: {e}")
    else:
        st.error("Please enter a valid question." if language == "English" else "يرجى إدخال سؤال صحيح.")